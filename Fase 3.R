...
grass_3 <- function(pEstado, pDia){
  ...
  #Probabilidades de transici?n entre estados de Nivel de precipitaci?n seg?n historico
  ...
  #Probabilidades de transici?n entre estados del cultivo seg?n historico
  ...
  #
  ##?Qu? pasa si no se interviene?
  #Estado del cultivo y de la lluvia se saca del hist?rico de los datos
  #Se crean las matrices de 50x50
  
  ##?Qu? pasa si se interviene?
  #Estado del cultivo se estima con una triangular y el de la lluvia del hist?rico de los datos
  #Se crean las matrices 50x50
  
  #240 matrices dado que es una para cada d?a con 4 alternativas de decisi?n
  
  #Se necesita una funci?n que cree las matrices de la lista
  ...
  Dt <- matrix(0,nrow=50,ncol=60) #Decisiones ?ptimas
  dimnames(Dt) <- list(estados,dias)
  
  Ft[,61] <- retornos[,61] #Retorno de la ?ltima ?poca (?poca fantasma)
  
  vaux = 0
  ret = 0
  for (dia in 60:1){ #Retornos para las dem?s ?pocas
    for (alt in A){
      for (est in estados){
        vaux <- sum(lista[[paste(dia,alt,sep=";")]][est,])
        ret <- 0 + vaux*Ft[est,dia+1]
        if (ret >= Ft[est,dia]){ #Se comprueba la optimalidad de la alternativa de decisi?n
          Ft[est,dia] = ret
          Dt[est,dia] = alt
        }
      }
